...
export const isProductFullyOutOfStock = (product: Product): boolean => {
  ...
  if (product.stock !== undefined && product.stock !== null && Number(product.stock) <= 0) return true;

  if (Array.isArray(product.variants) && product.variants.length > 0) {
    const activeVariants = product.variants.filter((v) => v.is_active !== false);
    if (activeVariants.length > 0) {
      return activeVariants.every((v) => v.stock !== undefined && v.stock !== null && Number(v.stock) <= 0);
    }
  }
